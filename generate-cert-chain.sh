...
#
# All use RSA 2048 with SHA256 (maps to SHA256withRSA in Java).
#
...
openssl req -x509 -new -nodes -key root_key.pem \
  -sha256 -days 3650 \
  -subj "/C=US/O=TestOrg1/OU=TestUnit/CN=${ROOT_CN}" \
...
openssl req -new -key inter_key.pem \
  -subj "/C=US/O=TestOrg1/OU=TestUnit/CN=${INTER_CN}" \
  -out inter.csr
...
openssl x509 -req -in inter.csr -CA root_cert.pem -CAkey root_key.pem \
  -CAcreateserial -out inter_cert.pem -days 3650 -sha256 \
...
openssl req -new -key leaf_key.pem \
  -subj "/C=US/O=TestOrg1/OU=TestUnit/CN=${LEAF_CN}" \
  -out leaf.csr
...
openssl x509 -req -in leaf.csr -CA inter_cert.pem -CAkey inter_key.pem \
  -CAcreateserial -out leaf_cert.pem -days 730 -sha256 \
...
# Full chain (leaf -> intermediate -> root) in PEM order typically used by clients
cat leaf_cert.pem inter_cert.pem root_cert.pem > chain_leaf_inter_root.pem

# CA chain (intermediate + root) useful for uploadCACertificate sequence
...
echo
echo "Signature algorithm in Java for all: SHA256withRSA"
